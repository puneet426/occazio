...
export default function CreateEventPage() {
  ...
  // 1. FIX: Get the user directly from Convex to check their REAL Pro status!
  const { data: currentUser } = useConvexQuery(api.users.getCurrentUser);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // 2. FIX: Check against the new 1 event limit for Free users
    ...
    >
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-5 md:flex-row justify-between mb-10">
        <div>
          ...
          {!hasPro && (
            <p className="text-sm text-muted-foreground mt-2">
              {/* 3. FIX: Display the correct 1 event limit */}
